export class AuthForm {
    constructor() {
        // DOM Elements
        this.form = document.getElementById("publicLoginForm");
    ...
    // Saare event listeners ko bind karne ka method
    initEvents() {
        // Password toggle feature
        document.addEventListener("click", (event) => this.handlePasswordToggle(event));

        // Form Submit (Login)
        this.form.addEventListener("submit", async (event) => {
            event.preventDefault();
            await this.submitAuth("/api/auth/login", "Login successful");
        });

        // Register Button Click
        if (this.registerBtn) {
            this.registerBtn.addEventListener("click", async () => {
                await this.submitAuth("/api/auth/register", "Registration sent. Admin approval pending.");
            });
        }
    }
    ...
    handlePasswordToggle(event) {
        ...
        const labelText = shouldShow ? "Hide password" : "Show password";
        button.setAttribute("aria-label", labelText);
        button.setAttribute("title", labelText);
    }

    // Central Auth Submission Logic
    async submitAuth(url, successMessage) {
        try {
            ...
            if (this.message) this.message.textContent = result.message || successMessage;

            if (url.endsWith("/login")) {
                window.location.href = this.getLoginRedirectUrl();
            }
        } catch (err) {
            if (this.message) this.message.textContent = err.message || "Request failed";
        }
    }

    // Redirect URL Helper Method
    getLoginRedirectUrl() {
...
// Class ko instantiate (initialize) karein jab DOM loaded ho
